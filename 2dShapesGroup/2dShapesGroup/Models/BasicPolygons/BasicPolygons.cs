...
{
    // ════════════════════════════════════════════════════════════════════
    //  GROUP 1 — BASIC POLYGONS
    //  Shapes: Square, Rectangle, Triangle, Pentagon, Hexagon,
    //          Parallelogram, Trapeze, Rhombus
    // ════════════════════════════════════════════════════════════════════
    ...
    // ────────────────────────────────────────────────────────────────────
    // TRIANGLE  (scalene — given three sides, drawn via Heron + coords)
    // ────────────────────────────────────────────────────────────────────
    /// <summary>
    ...
    public class Triangle : IShape
    {
        ...
        public void Draw(G g, RectangleF b, double[] v)
        {
            ...
            // cos(A) = (a²+c²-b²)/(2ac)  → but we need angle at vertex A
            // cos(angle_A) = (a²+c²-b²)/(2·a·c)  — angle between sides a and c
            ...
            PointF pC = new((float)(c * cosA), -(float)(c * sinA)); // up
            ...
            float offX = b.X + (b.Width  - drawW) / 2f; // minX subtracted in Scale()
            float offY = b.Y + (b.Height - drawH) / 2f; // minY subtracted in Scale()
    ...
    public class Pentagon : IShape
    {
        ...
        public void Draw(G g, RectangleF b, double[] v)
            => DrawRegularPolygon(g, b, 5);

        // Shared helper — draws any regular n-gon inside the padded square
        internal static void DrawRegularPolygon(G g, RectangleF b, int n)
        {
            ShapeGraphics.EnableAntiAlias(g);
    ...
    public class Hexagon : IShape
    {
        ...
        public void Draw(G g, RectangleF b, double[] v)
            => Pentagon.DrawRegularPolygon(g, b, 6);
    }
    ...
    // ────────────────────────────────────────────────────────────────────
    // TRAPEZE  (right / isosceles trapezoid)
    // ────────────────────────────────────────────────────────────────────
    /// <summary>
